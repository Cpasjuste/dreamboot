...
int InitSDCard() {
    ...
    if (sdc_init()) {
        dbglog(DBG_WARNING, "SD card not found.\n");
        return -1;
    }
    ...
    dbglog(DBG_INFO, "SD card initialized, capacity %"
            PRIu32
...
//				ddata->block_count += ddata->start_block;
                ...
                if (fs_fat_init()) {
                    dbglog(DBG_ERROR, "Could not initialize fs_fat!\n");
                    sd_dev[part].shutdown(&sd_dev[part]);
                } else {
                    ...
                    if (fs_fat_mount(path, &sd_dev[part], 0, part)) {
                        dbglog(DBG_ERROR, "Could not mount device as fatfs.\n");
                        sd_dev[part].shutdown(&sd_dev[part]);
                    }
                }

            } else {
                dbglog(DBG_ERROR, "Unknown filesystem: 0x%02x\n", partition_type);
                sd_dev[part].shutdown(&sd_dev[part]);
            }
...
int InitIDE() {
    ...
    if (g1_ata_init()) {
        dbglog(DBG_ERROR, "G1 ATA device not found.\n");
        return -1;
    }
    ...
    for (part = 0; part < MAX_PARTITIONS; part++) {
        ...
        if (!check_partition(buf, part) &&
            !g1_ata_blockdev_for_partition(part, use_dma, &g1_dev[part], &partition_type)) {
            ...
            if (is_ext2_partition(partition_type)) {
                ...
                if (fs_ext2_init()) {
                    dbglog(DBG_ERROR, "Could not initialize fs_ext2!\n");
                    ...
                    if (fs_ext2_mount(path, &g1_dev[part], FS_EXT2_MOUNT_READWRITE)) {
                        dbglog(DBG_ERROR, "Could not mount device as ext2fs.\n");
                    }
                }
